...
import { Calendar, Eye, EyeOff, AlertCircle, CheckCircle2, Check, X, Mail } from 'lucide-react'
...
export default function SignupPage() {
  ...
  // 'form' | 'verify' = waiting for email verification
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result: any = await api.auth.signup({
        email: form.email,
      ...
      if (result.requiresVerification) {
        ...
        // Fallback if backend skips verification (dev mode)
        ...
        <div className="w-full max-w-md page-enter">
          ...
          {stage === 'verify' ? (
            <div className="card p-8 text-center">
              ...
              <p className="font-body text-sm font-semibold text-ink-900 mb-5">{signedUpEmail}</p>
              <p className="font-body text-xs text-ink-400 mb-6">
                Click the link in the email to activate your account, then{' '}
                <Link href="/login" className="text-ink-700 underline underline-offset-2">log in</Link>.
              </p>
              <button
                type="button"
            ...
            <div className="card p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={loading || !form.email || !pwValid}
                  className={`btn-sage w-full justify-center py-3 mt-2 ${loading || !form.email || !pwValid ? 'opacity-60 cursor-not-allowed' : ''}`}>
                  {loading ? (
