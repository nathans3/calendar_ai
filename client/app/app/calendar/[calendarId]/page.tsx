...
function ExportModal({ calendarName, period, dayData, onClose }: {
  ...
  const [audience, setAudience]   = useState<'student' | 'sub' | null>(null)
  ...
  const handleStudentExport = () => {
    ...
    openPrint(`<!DOCTYPE html><html><head><title>${calendarName} — Student View</title>
    <style>body{font-family:Georgia,serif;max-width:780px;margin:40px auto;color:#0D0D0D;padding:0 20px}
    h1{font-size:26px;margin-bottom:4px}.sub{color:#707060;font-size:13px}.rl{color:#7A9E7E;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin:6px 0 30px}
    .day{margin-bottom:22px;padding-bottom:22px;border-bottom:1px solid #E8E8E0}.day-header{font-weight:700;font-size:14px;margin-bottom:8px}
    .lbl{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#A0A090;margin-top:9px}
  ...
  const handleSubExport = () => {
    const d = dayData[subDate]
    const dateLabel = format(parseISO(subDate), 'EEEE, MMMM d, yyyy')
    const lessonSection = d?.lessonPlan
      ? `<div class="section"><div class="sec-title">📚 Lesson Plan</div><div class="sec-body">${d.lessonPlan.replace(/\n/g, '<br>')}</div></div>`
      : `<div class="section empty">No lesson plan recorded for this day.</div>`
    const assessSection = d?.assessments
      ? `<div class="section warn"><div class="sec-title">📝 Assessment Today</div><div class="sec-body">${d.assessments}</div></div>` : ''
    const hwSection = d?.hw
    ...
    <style>body{font-family:Arial,sans-serif;max-width:750px;margin:40px auto;color:#0D0D0D;padding:0 24px}
    h1{font-size:22px;margin-bottom:2px;color:#0D0D0D}.sub{color:#707060;font-size:13px;margin-bottom:4px}
    .date-banner{background:#f0f5f0;border-left:4px solid #7A9E7E;padding:10px 16px;font-size:15px;font-weight:700;color:#2d4a30;margin:18px 0 24px;border-radius:0 6px 6px 0}
    .section{margin-bottom:18px;padding:14px 16px;border-radius:8px;border:1px solid #E8E8E0;background:#fafaf8}
    .section.warn{border-color:#f59e0b;background:#fffbeb}.section.closed{border-color:#93c5fd;background:#eff6ff}
    .section.notes{border-color:#d1d5db;background:#f9fafb}.section.empty{color:#A0A090;font-size:13px;font-style:italic}
    .sec-title{font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#555;margin-bottom:8px}
    .sec-body{font-size:14px;line-height:1.6;color:#1a1a1a}
    .footer{margin-top:36px;padding-top:16px;border-top:1px solid #E8E8E0;font-size:11px;color:#A0A090}
    @media print{body{margin:20px}}</style></head><body>
    <h1>Substitute Teacher Plan</h1>
    <div class="sub">${calendarName} &nbsp;·&nbsp; ${period}</div>
    <div class="date-banner">📅 ${dateLabel}</div>
    ${notesSection}${lessonSection}${assessSection}${hwSection}${deadlineSection}${teacherNotes}
    <div class="footer">Generated by Calendar AI &nbsp;·&nbsp; Print and leave on desk for substitute.</div>
    </body></html>`)
  }

  const commonStyles = `
    <style>body{font-family:Georgia,serif;max-width:800px;margin:40px auto;color:#0D0D0D;padding:0 20px}</style>
  `
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-7 max-w-sm w-full mx-4">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-xl text-ink-900">
              {audience === null ? 'Export' : audience === 'student' ? 'Export to Students' : 'Sub Plan'}
            </h2>
          </div>
          <div className="flex items-center gap-2">
            {audience !== null && !done && (
              <button onClick={() => setAudience(null)} className="font-body text-xs text-ink-400 hover:text-ink-700 transition-colors">← Back</button>
            )}
        ...
        {audience === null && (
          <>
            <p className="font-body text-sm text-ink-500 mb-5 leading-relaxed">Who are you exporting for?</p>
            <div className="space-y-3">
              <button onClick={() => setAudience('student')}
                className="w-full flex items-start gap-4 p-4 rounded-xl border border-ink-900/10 hover:border-sage hover:bg-sage/5 transition-all text-left group">
                <div className="w-9 h-9 bg-sage/10 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-sage/20 transition-colors">
                  <span className="text-lg">🎒</span>
                </div>
                <div>
                  <div className="font-body font-semibold text-sm text-ink-900">Students</div>
                  <div className="font-body text-xs text-ink-400 mt-0.5">Topics, homework, assessments & deadlines over a date range</div>
                </div>
              </button>
              <button onClick={() => setAudience('sub')}
                className="w-full flex items-start gap-4 p-4 rounded-xl border border-ink-900/10 hover:border-sage hover:bg-sage/5 transition-all text-left group">
                <div className="w-9 h-9 bg-sage/10 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-sage/20 transition-colors">
                  <span className="text-lg">📋</span>
                </div>
                <div>
                  <div className="font-body font-semibold text-sm text-ink-900">Substitute Teacher</div>
                  <div className="font-body text-xs text-ink-400 mt-0.5">Single-day plan with lesson, assessments & teacher notes</div>
                </div>
              </button>
            </div>
            <button onClick={onClose} className="btn-secondary w-full justify-center text-sm py-2.5 mt-4">Cancel</button>
          </>
        )}

        {/* Step 2a — Student export */}
        {audience === 'student' && !done && (
          <>
            <p className="font-body text-sm text-ink-500 mb-4 leading-relaxed">Export <strong className="text-ink-800">{calendarName}</strong> — topics, homework, tests, and deadlines.</p>
            <div className="mb-5">
            ...
            <div className="flex gap-3">
              <button onClick={() => setAudience(null)} className="btn-secondary flex-1 justify-center text-sm py-2.5">Back</button>
              <button onClick={handleStudentExport} className="btn-sage flex-1 justify-center text-sm py-2.5 gap-1.5"><Download className="w-3.5 h-3.5" />Export PDF</button>
            </div>
          </>
        )}

        {/* Step 2b — Sub plan export */}
        {audience === 'sub' && !done && (
          <>
            <p className="font-body text-sm text-ink-500 mb-4 leading-relaxed">Generate a single-day plan for a substitute teacher.</p>
            <div className="mb-4">
              <label className="font-body text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1.5 block">Date</label>
              <input type="date" value={subDate} onChange={e => setSubDate(e.target.value)} className="input-field w-full text-sm" />
            </div>
            <div className="mb-5">
              <label className="font-body text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1.5 block">Additional notes for sub <span className="font-normal text-ink-400 normal-case tracking-normal">(optional)</span></label>
              <textarea
                value={subNotes}
                onChange={e => setSubNotes(e.target.value)}
                placeholder="e.g. seating chart in top drawer, students may work in groups…"
                rows={3}
                className="input-field w-full text-xs resize-none"
              />
            </div>
            <div className="space-y-1.5 mb-6">
              {["Today's lesson plan","Any assessments or homework","Your extra notes","Print-ready format"].map(i => (
                <div key={i} className="flex items-center gap-2"><Check className="w-3.5 h-3.5 text-sage" /><span className="font-body text-xs text-ink-600">{i}</span></div>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setAudience(null)} className="btn-secondary flex-1 justify-center text-sm py-2.5">Back</button>
              <button onClick={handleSubExport} className="btn-sage flex-1 justify-center text-sm py-2.5 gap-1.5"><Download className="w-3.5 h-3.5" />Export PDF</button>
            </div>
          </>
        )}
        ...
        {done && (
          <div className="text-center py-4">
            ...
            <p className="font-body text-sm text-ink-600 mb-4">PDF ready — check your print preview.</p>
